...
W="${dims%x*}"; H="${dims#*x}"
SIZE_OPT=""
if [[ "$W" =~ ^[0-9]+$ && "$H" =~ ^[0-9]+$ && "$H" -gt "$W" ]]; then
  SIZE_OPT=":original_size=${W}x${H}"
  WIDTH=14
  ORI=竖屏
  FONT_SIZE=16
  MARGIN_V=96
...
[ -f "$OUT/zh.srt" ] || { echo "❌ 缺少中文字幕($OUT/zh.srt),先让 Claude 翻译"; exit 1; }
if [ -f "$OUT/en.srt" ]; then
  echo "检查中英字幕编号和时间轴..."
  python3 "$ROOT/bin/validate-srt-alignment.py" "$OUT/en.srt" "$OUT/zh.srt"
fi

# 有整句译文 zh.srt 就按视频朝向重新拆短:竖屏窄(14)、横屏宽(26),保证宽度合适且最新
...
"$FF_FULL" -y -loglevel error -stats -i video.mp4 \
  -vf "subtitles=$SUB:fontsdir=/System/Library/Fonts${SIZE_OPT}:force_style='FontName=PingFang SC,FontSize=$FONT_SIZE,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=1,Outline=1.6,Shadow=0,Alignment=2,MarginV=$MARGIN_V,ScaleX=100,ScaleY=100,Spacing=0'" \
