# Video Learning Workbench — Codex 工作说明
...
2. **你亲自翻译**(不要求用户配置付费翻译 API,且质量优先):
   ...
   - **时间轴和编号原样保留**,只替换文本行
   - 顺手**纠正 whisper 听错的术语**(如把 Claw→Codex、Cloud Code→Codex、solid model→Sonnet model);改了哪些要在回复里说明
   - 新术语随手补进 `glossary.md`,越攒越准
3. **烧字幕**:`bin/2-burn.sh <slug>` → `out.nosync/<slug>/<slug>.zh.mp4`(仅中文硬字幕)
   - 它会**自动按视频朝向把 zh.srt 拆短**(竖屏每行≤16、横屏≤26 全角,避免一屏太长挡画面)再烧录,**不用再手动拆短**。
   - (要手动微调可单独跑 `python3 bin/resplit-srt.py <zh.srt> <out.srt> <宽度>`)
4. **验收**:抽 2-3 帧不同时间点确认字幕全程正常、术语正确,再把成片路径给用户。
...
## 字幕设定
仅中文(非双语)、白字黑边、PingFang SC、贴底。样式在 `bin/2-burn.sh` 的 force_style 里。
...
## 多设备
- **新机器先跑一次 `bin/setup.sh`**:装 yt-dlp / ffmpeg-full / whisper-cpp + 下载本机模型。
- 模型和成片建议每台机器各自本地保存,不要同步大文件。
- 如果项目放在同步盘里,不要在多台机器上同时跑同一个输出目录,避免同步冲突。
