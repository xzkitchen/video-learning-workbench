...
3. Burn Chinese hard subtitles:

```bash
bin/2-burn.sh <slug>
```

4. Verify 2-3 frames at different timestamps. Confirm Chinese subtitles render, do not overflow badly, and key terms are correct.

5. Clean large source files only after verification if the user expects disk cleanup:
